...
import { fetchNutrients, type IngredientNutrients, type NutrientValue } from '@/lib/nutrition';
...
export async function updateRecipe(
  ...
  const allMissing: IngredientNutrients = {
    energy: 'missing', protein: 'missing', fat: 'missing', saturatedFat: 'missing',
    carbs: 'missing', fiber: 'missing', sugars: 'missing', salt: 'missing', sodium: 'missing',
  };
  ...
  const keys = Object.keys(perIngredient[0] ?? allMissing) as (keyof IngredientNutrients)[];
